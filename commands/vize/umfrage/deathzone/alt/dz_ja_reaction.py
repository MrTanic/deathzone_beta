...
class DzJaReactionAccountsCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.umfrage_db_path = "Datenbank/Umfrage_DB/Deathzone/dz_cw_umfrage.db"
        self.accounts_db_path = "Datenbank/Clash_DB/Link_DB/Deathzone_Accounts.db"
        self.clan_tag = "#2JLQVYQ8"
    ...
    @nextcord.slash_command(name='dz_ja_reaktionen', description='Zeigt Server-Nicknames und verknüpfte CoC-Namen, die mit Ja reagiert haben und im Clan sind')
    async def show_yes_reactions(self, interaction: nextcord.Interaction, message_id: str):
        ...
        for i in range(0, len(yes_user_ids), 10):
            embed = nextcord.Embed(title="Ja-Reaktionen im Clan", description="Nutzer, die mit Ja reagiert haben und im Clan sind", color=16532095)
...
def setup(bot):
    bot.add_cog(DzJaReactionAccountsCog(bot))