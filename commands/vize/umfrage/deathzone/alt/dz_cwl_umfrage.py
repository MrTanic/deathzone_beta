...
class DzCWLCommand(commands.Cog):
    ...
    @nextcord.slash_command(name='dz_cwl_announcement', description='Sendet eine Ankündigung für die kommende CWL', guild_ids=[884325196104884248])
    ...
    async def announce_cwl(self, interaction: nextcord.Interaction):
        channel = self.bot.get_channel(DZ_ANNOUNCEMENT_CHANNEL)  # ID des Ankündigungskanals aus channel.py
        if not channel:
            await interaction.response.send_message("Ankündigungskanal nicht gefunden.", ephemeral=True)
            return

        message_content = "Hallo zusammen <@&893503796569858059> !"
