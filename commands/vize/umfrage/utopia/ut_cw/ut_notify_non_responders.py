...
WEBHOOK_URL = os.getenv("UT_RESPONDERS")
...
async def notify_non_responders(bot, interaction: nextcord.Interaction, message_id: str):
    ...
    cursor.execute("SELECT user_id FROM ut_cw_umfrage WHERE message_id = ? AND reaction = 'nicht reagiert'", (message_id,))
    ...
    content = (
        ...
        f"Hier ist der Link zur Umfrage: \nhttps://discord.com/channels/{interaction.guild.id}/{UT_ANNOUNCEMENT_CHANNEL}/{message_id}"
    )
