...
WEBHOOK_URL = os.getenv("UT_CASTLE_FILLER")  # Ersetzen Sie dies mit dem Namen der Umgebungsvariablen für Ihren Webhook
...
async def choose_castle_fillers(bot, interaction, message_id, number_of_fillers):
    ...
    if not (has_permission(user, 'owner') or has_permission(user, 'utzvize')):
