...
from .ut_announcement_logic import send_announcement
from .ut_reactions_logic import save_and_create_embed
from .ut_notify_non_responders import notify_non_responders
from .ut_choose_castle_fillers_logic import choose_castle_fillers
from .ut_istdabei_logic import show_yes_responders
from .ut_monat_logic import get_non_responders, create_non_responder_embeds
...
class UtAnnouncementCog(commands.Cog):
    ...
    @nextcord.slash_command(name='ut', description='Verwalte Utopia Clan Operationen', guild_ids=SERVER)
    async def ut(self, interaction: nextcord.Interaction):
        pass  # Hauptbefehl, der als Container für die Sub-Befehle dient

    @ut.subcommand(name='announcement', description='Sendet eine Ankündigung für einen Clankrieg')
    ...
    @ut.subcommand(name='auswertung', description='Erstellt ein Embed aus gespeicherten Reaktionen')
    ...
    @ut.subcommand(name='notify', description='Benachrichtigt Mitglieder, die nicht auf die Umfrage reagiert haben')
    async def ut_notify_non_responders(self, interaction: nextcord.Interaction, message_id: str):
        if not await check_permissions(interaction):
            return
        await notify_non_responders(self.bot, interaction, message_id)

    @ut.subcommand(name='castle', description='Wählt zufällig Mitglieder aus, die mit "Ja" reagiert haben')
    async def ut_choose_castle_fillers(self, interaction: nextcord.Interaction, message_id: str, number_of_fillers: int):
        if not await check_permissions(interaction):
            return
        await choose_castle_fillers(self.bot, interaction, message_id, number_of_fillers)

    @ut.subcommand(name='istdabei', description='Zeigt alle User, die mit "Ja" reagiert haben, und deren Spieler Account Namen')
    async def ut_istdabei(self, interaction: nextcord.Interaction, message_id: str):
        if not await check_permissions(interaction):
            return
        await show_yes_responders(self.bot, interaction, message_id)

    @ut.subcommand(name='monat', description='Zeigt, wer im ausgewählten Monat und Jahr nicht reagiert hat.')
