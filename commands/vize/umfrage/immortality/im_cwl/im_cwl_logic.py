...
from bot.setup.discord.channel_ids import IM_CWL_CHANNEL
from .im_cwl_save_reaction import SaveIMCWLReactions

db_path = "datenbank/umfrage/immortality/im_cwl_umfrage.db"
member_db_path = "datenbank/member/immortality_member.db"
save_reactions = SaveIMCWLReactions(db_path, member_db_path)

async def save_and_evaluate_im_cwl(bot, interaction, message_id):
    ...
    try:
        ...
        cwl_channel = bot.get_channel(IM_CWL_CHANNEL)
    ...
    cursor.execute("SELECT user_id, reaction, saison_datum FROM im_cwl_umfragen WHERE message_id = ?", (message_id,))
    ...
    embed = nextcord.Embed(title="CWL Reaktionsauswertung", description=embed_description, color=0x32fcf6)
