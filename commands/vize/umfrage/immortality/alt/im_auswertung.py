...
LOG_FILE_PATH = "datenbank/logs/cw_date/im_cw_announcement_log.txt"

class ImCreateEmbedCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.db_path = "datenbank/umfrage/immortality/im_cw_umfrage.db"
        self.member_db_path = "datenbank/member/immortality_member.db"
    ...
    @nextcord.slash_command(name='im_auswertung', description='Erstellt ein Embed aus gespeicherten Reaktionen für Immortality', guild_ids=SERVER)
    async def create_embed(self, interaction: nextcord.Interaction, message_id: str):
        ...
        announcements_channel = self.bot.get_channel(IM_ANNOUNCEMENT_CHANNEL)
        ...
        embed = nextcord.Embed(title="Reaktionsauswertung für Immortality", description=embed_description, color=3341558)
...
def setup(bot):
    bot.add_cog(ImCreateEmbedCog(bot))