...
WEBHOOK_URL = os.getenv("IM_RESPONDERS")

async def notify_non_responders(bot, interaction: nextcord.Interaction, message_id: str):
    user = interaction.user
    if not (has_permission(user, 'owner') or has_permission(user, 'imvize')):
    ...
    cursor.execute("SELECT user_id FROM im_cw_umfrage WHERE message_id = ? AND reaction = 'nicht reagiert'", (message_id,))
    ...
    content = (
        ...
        f"Hier ist der Link zur Umfrage: \nhttps://discord.com/channels/{interaction.guild.id}/{IM_ANNOUNCEMENT_CHANNEL}/{message_id}"
    )
