...
db_path = "datenbank/umfrage/academy/ac_cw_umfrage.db"
member_db_path = "datenbank/member/academy_member.db"
save_reactions = SaveReactions(db_path, member_db_path)

async def save_and_create_ac_embed(bot, interaction: nextcord.Interaction, message_id: str):
    ...
    announcements_channel = bot.get_channel(AC_ANNOUNCEMENT_CHANNEL)
        ...
        for reaction in message.reactions:
            async for user in reaction.users():
                if user != bot.user:
                    ...
                    save_reactions.save_reaction_to_database(message_id, user_id, coc_name, reaction_type, umfrage_datum)
    ...
    embed = nextcord.Embed(title="Reaktionsauswertung", description=embed_description, color=2135246)
