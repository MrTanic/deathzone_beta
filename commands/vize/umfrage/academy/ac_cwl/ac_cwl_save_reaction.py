...
class SaveACCWLReactions:
    ...
    def save_reaction_to_database(self, message_id, user_id, coc_name, reactions, saison_datum):
        ...
        cursor.execute("DELETE FROM ac_cwl_umfragen WHERE message_id = ? AND user_id = ?", (message_id, user_id))
        ...
        for reaction in reactions:
            cursor.execute('''
                INSERT OR REPLACE INTO ac_cwl_umfragen (message_id, user_id, coc_name, reaction, saison_datum)
                VALUES (?, ?, ?, ?, ?)
