...
from bot.setup.discord.channel_ids import AC_CWL_CHANNEL
from .ac_cwl_save_reaction import SaveACCWLReactions

db_path = "datenbank/umfrage/academy/ac_cwl_umfrage.db"
member_db_path = "datenbank/member/academy_member.db"
save_reactions = SaveACCWLReactions(db_path, member_db_path)

async def save_and_evaluate_ac_cwl(bot, interaction, message_id):
    ...
    try:
        ...
        cwl_channel = bot.get_channel(AC_CWL_CHANNEL)
    ...
    cursor.execute("SELECT user_id, reaction, saison_datum FROM ac_cwl_umfragen WHERE message_id = ?", (message_id,))
    ...
    embed = nextcord.Embed(title="CWL Reaktionsauswertung", description=embed_description, color=2135246)
