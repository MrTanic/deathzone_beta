...
from .ac_save_reaction import SaveReactions

LOG_FILE_PATH = "datenbank/logs/cw_date/ac_cw_announcement_log.txt"

class AcCreateEmbedCog(commands.Cog):
    ...
    @nextcord.slash_command(name='ac_auswertung', description='Erstellt ein Embed aus gespeicherten Reaktionen für Academy', guild_ids=SERVER)
    async def create_embed(self, interaction: nextcord.Interaction, message_id: str):
        ...
        announcements_channel = self.bot.get_channel(AC_ANNOUNCEMENT_CHANNEL)
        ...
        try:
            ...
            umfrage_datum = self.save_reactions.load_cw_date_from_log(message_id, LOG_FILE_PATH) or datetime.now().strftime("%d.%m.%Y")
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT user_id, reaction, umfrage_datum FROM ac_cw_umfrage WHERE message_id = ?", (message_id,))
        ...
        embed = nextcord.Embed(title="Reaktionsauswertung Academy", description=embed_description, color=2141262)
        reactions = {'ja': [], 'nein': [], 'nicht reagiert': []}
...
def setup(bot):
    bot.add_cog(AcCreateEmbedCog(bot))