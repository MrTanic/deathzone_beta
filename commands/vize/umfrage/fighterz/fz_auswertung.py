...
from bot.setup.discord.channel_ids import FZ_ANNOUNCEMENT_CHANNEL
from bot.setup.discord.server_ids import SERVER
from .fz_save_reaction import SaveReactions

LOG_FILE_PATH = "datenbank/logs/cw_date/fz_cw_announcement_log.txt"

class FzCreateEmbedCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.db_path = "datenbank/umfrage/fighterz/fz_cw_umfrage.db"  # Pfad zur Fighterz Umfragen-Datenbank
        self.member_db_path = "datenbank/member/fighterz_member.db"  # Pfad zur Fighterz Mitglieder-Datenbank
    ...
    @nextcord.slash_command(name='fz_auswertung', description='Erstellt ein Embed aus gespeicherten Reaktionen für Fighterz', guild_ids=SERVER)
    async def create_embed(self, interaction: nextcord.Interaction, message_id: str):
        user = interaction.user
        if not has_permission(user, 'owner') and not has_permission(user, 'fzvize'):
            await interaction.response.send_message("Du hast nicht die erforderliche Berechtigung.", ephemeral=True)
            return

        announcements_channel = self.bot.get_channel(FZ_ANNOUNCEMENT_CHANNEL)
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT user_id, reaction, umfrage_datum FROM fz_cw_umfrage WHERE message_id = ?", (message_id,))
        ...
        embed = nextcord.Embed(title="Reaktionsauswertung Fighterz", description=embed_description, color=65416)  # Farbe für Fighterz
...
def setup(bot):
    bot.add_cog(FzCreateEmbedCog(bot))