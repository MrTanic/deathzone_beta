...
import pytz
from bot.setup.discord.channel_ids import CWL_CHANNEL, STATUS_CHANNEL_ID
from bot.setup.discord.user_ids import DEATHZONE_ROLE_ID

class CWLAnnouncementCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.announcement_channel_id = CWL_CHANNEL
    ...
    async def auto_announce(self):
        ...
        if now.day == 18 and now.hour == 20 and now.minute == 10:
    ...
    async def announce_cwl(self):
        ...
        message_content = f"Hallo zusammen <@&{DEATHZONE_ROLE_ID}> !"
...
def setup(bot):
    bot.add_cog(CWLAnnouncementCog(bot))